    ...
    while True:
        ...
        try:
            if target_score is target:
                raise ValueError(
                    f"Your target must be between 1 and 10, you provided\
                         {target_score}"
                    )
...
def display_results():
    ...
    """
    final_score = score if questions == 10

    if final_score > target_score:
        print(f"Sadly, your target score was {target_score} but you only got {final_score}.")
        print("You are not as clever as you think")
    if final_score == target_score:
        print(f"Well done! Your target score was {target_score} and you matched that it!")
        print(f" You scored {final_score}. You know exactly how clever you are!")
    if final_score < target_score:
        print(f"Congratulations! Your target was {target_score} but you scored {final_score}")
        print(f"You are much smarter than you think you are!")
